...
class wooght_control:
    ...
    def open_pingc(self):
        ...
        if is_jc:
            ...
            if(hk.get_color(self.num_arr['p_dc_zb'])!=self.cg.GOU_COLOR):
                ...
                time.sleep(0.1)
            #双击
            hk.mouse_dbclick([self.num_arr['p_dc_zb'][0]+250,self.num_arr['p_dc_zb'][1]])
            time.sleep(0.1)
            #设置数字
            if self.dc_num<8:
    ...
    def judge_cang(self):
        ...
        for i in range(40):
            ...
            if qr_color!='0xf0f0f0' and qr_color!=self.num_arr['open_qr_color']:
                #点击弹窗的确认
                hk.mouse_click(self.num_arr['jl_pc'])
                is_dj=True
                break
            time.sleep(0.1)
        #已经点击弹窗
        if is_dj:
            ...
            for i in range(3):
                ...
                qr_color=hk.get_color(self.num_arr['open_qr_zb'])
                #判断是提交成功 还是提交失败返回
                if qr_color!=self.num_arr['open_qr_color'] and qr_color!='0xf0f0f0':
                    #提交成功
                    return True
                else:
                    pd_num+=1
    ...
    def jianc_dk(self,num):
        if num==1:
            ...
            #多空坐标相距64像素
            dk_zb=[self.num_arr['dk_zb'][0]+64,self.num_arr['dk_zb'][1]]
